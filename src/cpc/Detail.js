...
const link = ""+host_custom+"/detail?X-Api-Key="+token+"&schedule_id="+schedule_id;
const link1 = ""+host_custom+"/listsortircpc?X-Api-Key="+token+"&schedule_id="+schedule_id;
...
fetch(link2).then((response) => response.json())
          ...
          {
setFoto(responseJson.data.list_da);
...
{
  isLoading(false);
  alert("CPC belum di pilih");
}
*/
else if(!Array.isArray(data_value))
{
 ...
 else if (  qrvalue1 == "" || qrvalue == "")
{
  ...
  var details = {
              ...
              'schedule_id' : schedule_id,
               'schedule_progress' :7,
               // 'document_no' :documenno,
                'document_bag_cpc' : qrvalue1,
               'document_seal_cpc' :qrvalue,
                'document_sticker_cpc' : dokstiker,
                 'schedule_is_cis' :cis,
               //'schedule_is_cpc' :cpc,     
};
...
fetch(""+host_custom+"/simpan_sortir", 
        {
            method: 'POST',
            headers: 
            {
...
return (
   <View style={styles.menu} >
          ...
          <Text style={styles.Judulbox}>Sortir</Text>
          </View>
          <View  style={{ alignItems: 'flex-end',width:'50%'}}>
           <TouchableOpacity style={{backgroundColor:Bg1_,flex: 1,
     height: 50,
    alignItems: 'center',
    justifyContent: 'center',width:50,elevation: 8,borderRadius:8}} onPress={() => Pindah_halaman('Cpc_sortir')}>
...
<DataTable.Row key={i} tyle={{textAlign:'right',alignItems:'flex-end',flex:1}}>
    ...
    <DataTable.Cell >
<Text style={{fontSize:12,color:'#000060',fontWeight:'normal'}}>{us.kurs}</Text>
</DataTable.Cell>
...
style={{ color: '#FFFFFF',
  ...
  <DataTable.Header> 
           ...
           <DataTable.Title style={{flex:4,justifyContent:'center'}}><Text style={{ fontSize:16,color:'green',fontWeight:'100'}}>Total IDR : {Array.isArray(data_value)
        ?  numberWithCommas(data_value.reduce((total, val) => total + parseInt(val.list_denom * val.list_value * val.nilai_tukar),0)
):0}</Text></DataTable.Title>
...
formBody = formBody.join("&");
fetch(""+host_custom+"/delete_sortir", 
        {
            method: 'POST',
            headers: 
            {
...
return (
   <View style={styles.menu} >
          ...
          <TouchableOpacity style={styles.Box} onPress={() => Pindah_halaman('Foto_da',schedule_id)}>
...
style={{ color: '#FFFFFF',
    ...
    fontSize: 12}}>X
      ...
      <Image source={{uri:us.file_da}} style={styles.logo}></Image>
            <Text numberOfLines={2} style={styles.stathijau}>{us.no_da}
</Text>
      </View>)
}) :    <Text numberOfLines={2} style={styles.statmerah}>Belum Complete
</Text>}
...
<View style={styles.menu}>
  <Text numberOfLines={2} style={styles.wtext}>Kode Schedule :
{ schedule_kode}
</Text>
</View>
<Value_denom/>
