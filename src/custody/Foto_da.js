...
const Foto_da = ({navigation,route}) => {
...
isLoading(true);
if (imageSource == null || noda=='') 
{Alert.alert("Foto tidak dan NO DA tidak boleh kosong");
isLoading(false); }
else {
   isLoading(true);
            const dataa = new FormData();
            dataa.append('file_da', {
                type: "image/jpeg",
                name: new Date()+'.png',
                uri: imageSource
              });
                dataa.append('schedule_id',schedule_id);
                dataa.append('no_da',noda);
                dataa.append('user',user);
         fetch(""+host+"/list_da/add", {
            method: 'POST',
            headers: 
            {
  ...
  if(responseJson.status == true)
          {
          //ascdata();
           // navigation.navigate('Custody_detail_job',{ schedule_id : schedule_id});
...
<View style={{padding:10,paddingVertical:30}} >
 ...
 <TextInput style={styles.inputBox}
      ...
      autoCorrect={false}
       ...
       onChangeText={value => setnoda(value)}
